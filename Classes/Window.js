function Window(id, field, domId) {
    ...
    this.appendElement = function (type) {
        var elem = presenter.createModule(type, this, this._id + '_' + counter);
    ...
    this.deleteElement = function (id) {
        var elem = this.children[id];
        console.log(elem,id);
        console.log(this.connections)
        for (var c in this.connections)
            for (var cc in this.connections[c]){                
                if (c == elem._id || cc == elem._id) {
                    this.dom.removeChild(this.connections[c][cc]);
                    delete this.connections[c][cc];
                }
            }
        console.log(this.connections)
    };
    ...
    this.getLineParams = function (obj1, obj2) {
        ...
        var y1 = offset1.top - margin1.top + size1.height / 2 - window.getComputedStyle(Header).height.replace('px', '') - parseInt(window.getComputedStyle(Field).paddingTop.replace('px', '')) * 6 - (this.dom.getAttribute('data-y') || 0);
        // top right
        var x2 = offset2.left - margin2.left - parseInt(window.getComputedStyle(Field).paddingLeft.replace('px', '')) + size2.width / 2 - (this.dom.getAttribute('data-x') || 0);
        var y2 = offset2.top - margin2.top / 2 + size2.height / 2 - window.getComputedStyle(Header).height.replace('px', '') - parseInt(window.getComputedStyle(Field).paddingTop.replace('px', '')) * 6 - (this.dom.getAttribute('data-y') || 0); //- window.getComputedStyle(Field).top.replace('px','')
